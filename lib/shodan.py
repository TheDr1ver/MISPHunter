...
def shodan_search(misphunter, search):
    _log.debug(f"Running SHODAN search {search}")
    raw = {}
    ips = []
    shodan_api = Shodan(misphunter.shodan_key)

    if misphunter.do_it_live:
        _log.debug(f"Using LIVE API query to search SHODAN...")
        # Get rate_limit for this service
        rate_limit = misphunter.shodan_rate
        try:
            ...
            misphunter.shodan_api_counter+=1
            _log.debug(f"\n\n#### TOTAL SHODAN API CALLS NOW {misphunter.shodan_api_counter}! \n\n")
            _log.debug(f"SHODAN got a 200 OK for {search}!")

        except shodan_api.APIError:
            _log.error(f"Error - API timed out. Trying again...")
            try:
                helper.rate_respect(misphunter.search_time, rate_limit+1)
                misphunter.search_time = time()
                raw = shodan_api.search(query=f"{search}")
                misphunter.shodan_api_counter+=1
                _log.debug(f"\n\n#### TOTAL SHODAN API CALLS NOW {misphunter.shodan_api_counter}! \n\n")
                ...
                _log.error(f"Error searching SHODAN for query {search}: {e}")
                # raise Exception(e)
                return ips
...
def shodan_search_ip(misphunter, event, seed, host_obj):
    ...
    if misphunter.do_it_live:
        ...
        _log.debug(f"Using LIVE API query to reach out to Shodan and get IP data...")
        try: 
            # Sleep {rate_limit} seconds for each time the search is run
            helper.rate_respect(misphunter.search_time, rate_limit)
            misphunter.search_time = time() 
...
def shodan_search_cert_hosts(misphunter, cert):
    # hunt_type = 'censys-v2'
    _log.debug(f"Getting raw JSON data from SHODAN for {cert}")
    raw = {}
    ips = []
    shodan_api = Shodan(misphunter.shodan_key)
    ...
    if misphunter.do_it_live:
        ...
        _log.info(f"Using LIVE API query to reach out to SHODAN and find hosts associated with cert {cert}...")
        # Sleep {rate_limit} seconds for each time the search is run
        rate_limit = misphunter.shodan_rate
        try:
            # rate_limit = helper.get_rate_limit(misphunter, 'censys')
            helper.rate_respect(misphunter.search_time, rate_limit)
            misphunter.search_time = time() 
            ...
            misphunter.shodan_api_counter+=1
            _log.debug(f"\n\n#### TOTAL SHODAN API CALLS NOW {misphunter.shodan_api_counter}! \n\n")
            _log.debug(f"SHODAN got a 200 OK for {cert}!")
            ...
            try:
                ...
                _log.error(f"Error searching SHODAN for certificate {cert}: {e}")
                return ips
